...
6. Commit the focused news change on a `codex/news-*` branch and open a pull request. Wait for the required `validate` and `Vercel` checks, keep the branch current with `main`, and merge through the normal pull request flow. Do not use administrator bypasses. Merging to `main` deploys `suped/openbimi` through the existing Vercel Git integration.
